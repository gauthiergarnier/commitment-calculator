...
import { generateMonthlyUsage, calculateOptimalCommitments } from "@/lib/variation-patterns"
...
export default function Home() {
  ...
  const [supportLevel, setSupportLevel] = React.useState<SupportLevel>('No Support')
  const [freeUserLicenses, setFreeUserLicenses] = React.useState(0)
  const [selectedTemplate, setSelectedTemplate] = React.useState<TemplateType>('5-marketing-websites')
  const [averageMonthlyCost, setAverageMonthlyCost] = React.useState(5000)
  ...
  React.useEffect(() => {
    const monthlyUsage = generateMonthlyUsage(averageMonthlyCost, variationPattern)
    const yearCommitments = calculateOptimalCommitments(monthlyUsage)
